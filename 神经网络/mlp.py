# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 19:09:27 2020
...
testSet=dataset_test.iloc[:,0:14]
index=dataset_test.iloc[:,[14]]
X_test=np.array(testSet)

#调参
clf = MLPClassifier(activation='relu',solver='adam',alpha=1e-4,hidden_layer_sizes=(50,),max_iter=1200,early_stopping=True,batch_size=32)
#clf = MLPClassifier(activation='relu',solver='adam',alpha=1e-4,hidden_layer_sizes=(50,30,20),max_iter=1200,early_stopping=True,batch_size=32)
#clf = MLPClassifier(activation='relu',solver='adam',alpha=1e-4,hidden_layer_sizes=(60,30),max_iter=1200,early_stopping=True,batch_size=32)
#clf = MLPClassifier(activation='relu',solver='adam',alpha=1e-4,hidden_layer_sizes=(20,),max_iter=100,early_stopping=True,batch_size=32)

clf.fit(trainingSet, trainingLabels)

print('训练准确率：',clf.score(trainingSet, trainingLabels))
#进行交叉验证
...
submission.to_csv('E:\\data_mining\\eye_classification\\result\\neural_network\\MLP_predict3.csv',index=False)


'''
#不使用库，从头开始实现
# 设置网络参数
layer =[14,3,1] # 设置层数和节点数
Lambda = 0.005 # 正则化系数
alpha = 0.2 # 学习速率
...
result.to_csv('./result/NN/network.csv',index=False)
